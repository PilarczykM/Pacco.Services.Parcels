...
public class GetParcelsHandler : IQueryHandler<GetParcels, IEnumerable<ParcelDto>>
{
	...
	public async Task<IEnumerable<ParcelDto>> HandleAsync(GetParcels query, CancellationToken cancellationToken = default)
	{
		...
		var parcels = await documents.ToListAsync(cancellationToken: cancellationToken);
